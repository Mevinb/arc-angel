...
class TerminalUI:
    ...
    def run(self) -> None:
        ...
        console.print(f"  JARVIS ready — safety mode: "
                      f"[bold]{self.app.guard.mode}[/bold]. "
                      "Type /help for commands.\n")
        if self.app.profile.is_placeholder():
            console.print("[dim]Tip: run `jarvis init` to fill in your profile "
        ...
        while True:
            ...
            console.print()
            status = console.status("[bold blue]jarvis is thinking…[/bold blue]")
            self._status = status
            status.start()
            try:
