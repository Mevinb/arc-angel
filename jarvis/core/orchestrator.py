"""Phase 2 — Orchestrator: the JARVIS agent loop.
...
def build_system_prompt(profile: Optional[Profile],
    ...
    ``skills`` (optional) is a :class:`jarvis.skills.index.SkillLibrary`. When
    ...
    parts: List[str] = [
        f"You are JARVIS, a personal AI assistant. Today is {today}.",
    ...
    if profile is not None and not profile.is_placeholder():
        ...
        parts.append("\n(User profile is still the placeholder template — suggest "
                     "`jarvis init` to fill it in when relevant.)")
