...
logger = logging.getLogger("arc.skills")
...
def skill_library_from_env(project_root: Path | str | None = None,
    ...
    Respects an explicit ``skills_root`` argument or ``ARC_SKILLS_ROOT`` in
    ...
    skills_root = (
        env.get("skills_root")
        or os.environ.get("ARC_SKILLS_ROOT")
        or os.environ.get("JARVIS_SKILLS_ROOT")  # legacy fallback
    )
    if not skills_root:
        # honour the same .env file the config loader uses (ARC_* preferred,
        # JARVIS_* honoured for pre-rename setups)
        env_file = root / ".env"
        if env_file.is_file():
            jarvis_fallback: str | None = None
            for raw_line in env_file.read_text(encoding="utf-8").splitlines():
                line = raw_line.strip()
                if line.startswith("ARC_SKILLS_ROOT"):
                    skills_root = line.partition("=")[2].strip().strip("'\"")
                    break
                if jarvis_fallback is None and line.startswith("JARVIS_SKILLS_ROOT"):
                    jarvis_fallback = line.partition("=")[2].strip().strip("'\"")
            else:
                # no ARC_ entry found — use JARVIS_ fallback if present
                if jarvis_fallback:
                    skills_root = jarvis_fallback
    if skills_root:
