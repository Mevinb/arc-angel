"""Application facade — wires every ARC subsystem together.

One ``ArcApp`` instance owns the config, database, profile, LLM router,
...
logger = logging.getLogger("arc.app")
...
class ArcApp:
    ...
    def _resolve_skills_root(self) -> Optional[Path]:
        ...
        An explicit ``skills.root`` in config (or ``ARC_SKILLS_ROOT`` in the
...
def create_app(approver: Optional[Approver] = None,
               quiet: bool = False) -> ArcApp:
    """Convenience factory (loads config from the usual locations)."""
    return ArcApp(approver=approver, quiet=quiet)


__all__ = ["ArcApp", "create_app"]
