...
logger = logging.getLogger("arc.llm")
...
class LLMRouter:
    ...
    def health(self) -> Dict[str, Any]:
        """Check gateway reachability; returns status dict for `arc doctor`.

        Also reports which configured role -> model IDs resolve to models the
        gateway actually offers, so ARC (and the user) can see exactly which
